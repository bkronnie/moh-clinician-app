...
$sqlC = "WITH req(table_name,column_name) AS (VALUES ('weeklyreport','days_worked'),('weeklyreport','submitted_on'),('weeklyreport','submit_status'),('weeklyreport','report_status'),('users','access_scope'),('users','rights'),('employees','employee_number'),('employees','date_of_birth'),('employees','phone_number'),('employees','facility'),('employees','department'),('staffleave','leave_type_id'),('staffleave','return_date'),('staffleave','leave_status')) SELECT table_name || '|' || column_name FROM req r WHERE NOT EXISTS (SELECT 1 FROM information_schema.columns c WHERE c.table_schema='clinician_app' AND c.table_name=r.table_name AND c.column_name=r.column_name) ORDER BY 1,2;"
