...
func RouteReports(r *gin.RouterGroup, db *sql.DB, sessionManager *scs.SessionManager) {
	v := r.Group("/reports")
	{
		...
		v.GET("/analysis/week-days", func(c *gin.Context) { handlers.HandlerReportsAnalysisWeekDays(c, db, sessionManager) })
		v.GET("/analysis/day-staff", func(c *gin.Context) { handlers.HandlerReportsAnalysisDayStaff(c, db, sessionManager) })
		v.GET("/analysis/day-staff/entry", func(c *gin.Context) { handlers.HandlerReportsAnalysisDayStaffEntry(c, db, sessionManager) })
		v.POST("/analysis/day-staff/review", func(c *gin.Context) { handlers.HandlerReportsAnalysisDayStaffReview(c, db, sessionManager) })
		v.POST("/analysis/day-staff/approve-all", func(c *gin.Context) { handlers.HandlerReportsAnalysisDayApproveAll(c, db, sessionManager) })
	...
	managerOrAdmin.Use(middleware.RequireRoles(db, sessionManager, "National Admin", "Facility Admin"))
	{
		...
		managerOrAdmin.POST("/analysis/submit-day", func(c *gin.Context) { handlers.HandlerReportSubmissionSubmitDay(c, db, sessionManager) })
...
func RouteCustomization(r *gin.RouterGroup, db *sql.DB, sessionManager *scs.SessionManager) {
	...
	v.Use(middleware.RequireRoles(db, sessionManager, "National Admin"))
	{
		...
		v.POST("/target/save", func(c *gin.Context) { handlers.HandlerCustomizationRoleTargetSave(c, db, sessionManager) })
