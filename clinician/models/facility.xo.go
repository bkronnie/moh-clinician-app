...
type Facility struct {
	...
	FLevel    string        `json:"f_level"`    // f_level
	FLg       sql.NullInt64 `json:"f_lg"`       // f_lg
...
func (f *Facility) Insert(ctx context.Context, db DB) error {
	...
	const sqlstr = `INSERT INTO public.facilities (` +
		...
		`) RETURNING id`
	// run
	logf(sqlstr, f.FName, f.FLevel, f.FLg, f.CreatedBy, f.CreatedOn)
	if err := db.QueryRowContext(ctx, sqlstr, f.FName, f.FLevel, f.FLg, f.CreatedBy, f.CreatedOn).Scan(&f.ID); err != nil {
		return logerror(err)
	}
	// set exists
	f._exists = true
...
func (f *Facility) Update(ctx context.Context, db DB) error {
	...
	const sqlstr = `UPDATE public.facilities SET ` +
		...
		`WHERE id = $6`
	// run
	logf(sqlstr, f.FName, f.FLevel, f.FLg, f.CreatedBy, f.CreatedOn, f.ID)
	if _, err := db.ExecContext(ctx, sqlstr, f.FName, f.FLevel, f.FLg, f.CreatedBy, f.CreatedOn, f.ID); err != nil {
		return logerror(err)
	}
	return nil
}
...
func (f *Facility) Upsert(ctx context.Context, db DB) error {
	...
	// run
	logf(sqlstr, f.ID, f.FName, f.FLevel, f.FLg, f.CreatedBy, f.CreatedOn)
	if _, err := db.ExecContext(ctx, sqlstr, f.ID, f.FName, f.FLevel, f.FLg, f.CreatedBy, f.CreatedOn); err != nil {
		return logerror(err)
	}
	// set exists
	f._exists = true
...
func FacilityByID(ctx context.Context, db DB, id int) (*Facility, error) {
	...
	if err := db.QueryRowContext(ctx, sqlstr, id).Scan(&f.ID, &f.FName, &f.FLevel, &f.FLg, &f.CreatedBy, &f.CreatedOn); err != nil {
