...
) THEN CREATE ROLE clinician_app LOGIN PASSWORD 'root';
END IF;
END $$;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_employees_employee_number_unique ON clinician_app.employees(employee_number)
...
FROM clinician_app.users u
    ...
    LEFT JOIN clinician_app.rights r ON LOWER(TRIM(u.rights)) = LOWER(TRIM(r.rights))
...
CREATE INDEX IF NOT EXISTS idx_employee_profile_changes_employee_changed_on ON clinician_app.employee_profile_changes(employee_id, changed_on DESC);
...
CREATE INDEX IF NOT EXISTS idx_customization_change_log_on_changed_on ON clinician_app.customization_change_log(changed_on DESC);
CREATE INDEX IF NOT EXISTS idx_customization_change_log_entity ON clinician_app.customization_change_log(entity_type, entity_id, changed_on DESC);
...
CREATE INDEX IF NOT EXISTS idx_report_data_elements_active_position ON clinician_app.report_data_elements(is_active, position);
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_weeklyreport_employee_hospital_start ON clinician_app.weeklyreport(employee, hospital, start)
...
CREATE INDEX IF NOT EXISTS idx_weeklyreport_facility_week_review_flow ON clinician_app.weeklyreport(
    hospital,
    start,
...
INSERT INTO clinician_app.leavetypes (leave_type_id, leave_type_name, description)
...
INSERT INTO clinician_app.departments (id, d_name)
VALUES (1, 'Surgery'),
    (2, 'Internal Medicine'),
    (3, 'Paediatrics'),
